...
<div class="f-col flex-1">
    ...
    <div class="icon">
      <router-link to="/" class="router-link" v-slot="{ navigate }">
        <div id="logo-container" @click="navigate" role="link">
          <img alt="logo" src="/images/app_icon.png" height="100" width="100" />
        </div>
      </router-link>
    </div>
    ...
    <div class="flex-4 f-row website-name-container" v-else>
      <h1>Jpec's Home</h1>
    ...
    <div class="flex-1" v-if="window.width <= 600">
            <div class="module-link-container">
        <font-awesome-icon icon="bars" size="2x" @click="showNav = !showNav"/>
      </div>
    </div>
  </div>
